...
const Submission = require('../models/submission');  // Your Submission model
const User = require('../models/User');               // Import User model
const { authenticate } = require('../middleware/auth');


// In your backend route file (e.g. submissions.js or auth.js)

router.post('/', authenticate, async (req, res) => {
  console.log("Received /api/submit POST request");
  ...
  try {
    ...
    if (allTestCases.length === 0) return res.status(400).json({ output: "No test cases found" });
    ...
    // Run test case for python, cpp, c using docker exec
    ...
    // Run all test cases sequentially (visible + hidden)
    const results = [];
    ...
    // Save submission and update user if all test cases passed
    if (passedAll) {
      try {
        ...
        console.log(`Saved successful submission for user ${req.user._id} and problem ${problemId}`);

        // Update user's solvedProblems array if not already solved
        ...
        console.error('Error saving submission or updating user:', dbError);
      }
    }
