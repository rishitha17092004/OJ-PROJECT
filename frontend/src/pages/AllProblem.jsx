...
export default function AdminProblemList() {
  ...
  useEffect(() => {
    const fetchProblems = async () => {
      try {
        const res = await axios.get('https://3.95.228.48:5000/api/problems');
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this problem?")) {
      try {
        await axios.delete(`https://3.95.228.48:5000/api/problems/delete/${id}`);
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`https://3.95.228.48:5000/api/problems/edit/${editProblem._id}`, editProblem);
